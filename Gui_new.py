...
DEMO_IMAGE = R"colored_accident_002_02_54_image.png"
actions = list({'accident': 0, 'call': 1, 'doctor': 2, 'help': 3, 'hot': 4, 'lose': 5, 'pain': 6, 'thief': 7}.keys())
DEMO_VIDEO = r"lose_026_01.AVI"
mp_drawing = mp.solutions.drawing_utils
mp_holistic = mp.solutions.holistic
holistic = mp_holistic.Holistic()
model_path = r"best_cnn_model.h5"
...
st.title('Indian Emergency Sign Language Detection System')
...
if app_mode =='About App':
    ...
    st.sidebar.markdown('---')
    CNN = st.sidebar.button('CNN_Model')
    LSTM = st.sidebar.button('LSTM_Model')
    st.sidebar.markdown('---')
    ...
    min_tracking_confidence=tracking_confidence ) as holistic:
        ...
        while vid.isOpened():
            ...
            kpi3_text.write(f"<h1 style='text-align: center; color: red;'>{actions[np.argmax(res)]}</h1>", unsafe_allow_html=True)
            kpi4_text.write(f"<h1 style='text-align: center; color: red;'>{mod}</h1>", unsafe_allow_html=True)
    ...
    min_detection_confidence=detection_confidence ) as holistic:
            ...
            cv2.putText(frame, actions[np.argmax(res)], (3,30),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2, cv2.LINE_AA)
            #Dashboard
            kpi1_text.write(f"<h1 style='text-align: center; color: red;'>{frame.shape[0]}</h1>", unsafe_allow_html=True)
            kpi2_text.write(f"<h1 style='text-align: center; color: red;'>{frame.shape[1]}</h1>", unsafe_allow_html=True)
            kpi3_text.write(f"<h1 style='text-align: center; color: red;'>{actions[np.argmax(res)]}</h1>", unsafe_allow_html=True)
            kpi4_text.write(f"<h1 style='text-align: center; color: red;'>{mod}</h1>", unsafe_allow_html=True)
